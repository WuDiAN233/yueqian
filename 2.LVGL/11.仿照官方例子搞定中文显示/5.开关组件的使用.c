...
/*
    开关组件的使用
       1.判断开关组件的两种状态
           开启  lv_obj_has_state(sw1, LV_STATE_CHECKED)
           关闭  lv_obj_has_state(sw1, LV_STATE_CHECKED)
*/
lv_obj_t * sw1;
// 开关组件的事件响应函数
void sw1_cb(lv_event_t * e)
{
    printf("开关组件被点击!\n");
    // 判断开关的状态
...
int main(void)
{
    ...
    // 创建开关组件
    sw1 = lv_switch_create(lv_scr_act()); // 按钮就会在开发板液晶屏上显示

    // 修改开关组件的坐标位置
    lv_obj_set_pos(sw1, 100, 140);
    // 修改开关组件大小
    lv_obj_set_size(sw1, 100, 50);

    // 给开关组件添加事件响应函数:  LV_EVENT_CLICKED枚举值表示按钮点击事件

    lv_obj_add_event_cb(sw1, sw1_cb, LV_EVENT_CLICKED, NULL);
