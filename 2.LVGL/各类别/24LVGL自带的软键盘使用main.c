...
/*
    软键盘的使用
       1.默认软键盘不需要设置坐标和大小，默认在屏幕正下方显示
       2.软键盘必须跟文本输入框绑定，绑定之后软键盘输入的内容才能在文本输入框中显示
       3.软键盘不能同时绑定多个输入框，解决方法
         在输入框的事件响应函数中分别重复绑定软键盘即可
...
int main(void)
{
    ...
    // 把软键盘跟文本输入框绑定：逻辑不正确
    /*
        软键盘同时绑定多个输入框，最终只有最后一个绑定的有效
    */
    // lv_keyboard_set_textarea(kb1, ta1);
    // lv_keyboard_set_textarea(kb1, ta2);

    // 解决方法：给输入框添加事件响应函数，事件类型--》光标进入事件LV_EVENT_FOCUSED
