...
/*
    按钮矩阵的使用
       1.什么是按钮矩阵： 多个按钮组成的集合
       2.判断按钮矩阵，你点击的究竟是哪个按钮？
         给按钮矩阵对象添加事件响应函数

*/

lv_obj_t * btm; // 按钮矩阵的地址
const char * btnmap[] = {
    "1", "2", "3", "\n", "4", "5", "6", "\n", "7", "8", "9", "\n", "#", "0", "%", "",
}; // 四行按钮，最后一行的最后一个必须传递空字符串""
...
// 按钮矩阵对应的事件响应函数
void btm_cb(lv_event_t * e)
{
    uint8_t id;
    printf("彭老师点击了某个按钮\n");
    // 获取你点击的那个按钮对象的地址
    lv_obj_t * somebt = lv_event_get_target(e);

    // 获取你点击的那个按钮对象的ID，ID从0开始
    id = lv_btnmatrix_get_selected_btn(btm);
    printf("你点击的那个按钮的id是 %d\n", id);

    // 获取你点击的那个按钮对象的字面值(按钮上文字内容)
    char * msg = lv_btnmatrix_get_btn_text(btm, id);
    printf("你点击的那个按钮文字内容是: %s\n", msg);
}
...
int main(void)
{
    ...
    // 创建按钮矩阵
    btm = lv_btnmatrix_create(lv_scr_act());

    // 调整大小
    lv_obj_set_size(btm, 400, 480);

    // 调整按钮矩阵上按钮文字大小
    lv_obj_set_style_text_font(btm, &lv_font_montserrat_48, LV_STATE_DEFAULT);

    // 设置按钮矩阵中的按钮文字内容，按钮总的个数
    lv_btnmatrix_set_map(btm, btnmap); // btnmap我定义的数组的名字,用来存放按钮的字面值(按钮上的文字)

    // 给按钮矩阵添加事件响应函数
    lv_obj_add_event_cb(btm, btm_cb, LV_EVENT_CLICKED, NULL);
