...
/*
     详细讲解post请求如何发送
	    1.http发送请求的过程就是用的tcp协议
		  访问某个网址：浏览器是tcp客户端
		                现在我们自己写代码，就用tcp客户端的代码替换掉浏览器的功能
        2.关键点一
		     服务器的ip
			    答案：gethostbyname("https://jmlstqxxcx.market.alicloudapi.com/weather/query/history")函数获取天气预报服务器的ip地址
	            注意：https://要去掉
				      .com后面的路径名去掉
             服务器的端口号
			    端口号：固定是80，因为http协议就是80
		  关键点二
		     post请求发送格式分析
		  关键点三
             发送http请求，如果这个请求成功，服务器会自动回复应答信息给客户端
			 客户端只需要调用recv()函数接收服务器的应答信息即可
*/ 
