...
const char *btns[]={"继续","关闭",""};  //给消息盒子两个按钮设置字面值

// 登录按钮的事件响应函数
// 参数：lv_event_t是个结构体：明天早上讲解这个结构体的用法
void bt1_cb(lv_event_t * e)
{
    //获取输入框输入的用户名和密码，判断用户名和密码是否正确
    //假设：正确的用户名张三
    //      正确的密码123
    const char* name = lv_textarea_get_text(ta1);
    const char* passwd = lv_textarea_get_text(ta2);
    if(strcmp(name,"张三")==0 && strcmp(passwd,"123")==0)
    {
        // 显示主界面
        show_main();
        // 关闭当前的登录界面
    ...
    {
        //弹出消息盒子，提示用户
        lv_obj_t *msgbox1=lv_msgbox_create(lv_scr_act(),"出错了","您输入的用户名或密码有误",btns,true);
        //给消息盒子添加中文样式
        lv_obj_add_style(msgbox1,mystyle,0);
        //给消息盒子设置坐标
        lv_obj_set_pos(msgbox1,300,50);
    }
...
{
    //创建百问网拼音汉字输入法软键盘
    kb1 = lv_100ask_pinyin_ime_create(loginwin);
    //软键盘使用中文样式
    lv_obj_add_style(kb1, mystyle, 0);
    //设置一下软键盘的坐标
    lv_obj_set_pos(kb1,0,1000);
    // 把软键盘跟第一个输入框关联
    lv_100ask_pinyin_ime_attach(kb1, e->target);
}
...
int show_login()
{
    ...
    // 创建图片对象-->把登录界面作为父窗口
    lv_obj_t * img1 = lv_img_create(loginwin);
    // 指定图片的路径名: 由于我之前lv_conf.h第579行设置过卷标是S
    lv_img_set_src(img1, "S:/1.jpg"); // 路径名前面必须要有卷标
...
int show_main()
{
    ...
    // 创建图片对象-->把登录界面作为父窗口
    lv_obj_t * img1 = lv_img_create(mainwin);

    // 指定图片的路径名: 由于我之前lv_conf.h第579行设置过卷标是S
    lv_img_set_src(img1, "S:/2.jpg"); // 路径名前面必须要有卷标
...
int main(void)
{
    ...
    //创建中文汉字样式
    mystyle=create_chinese_style("/font/simkai.ttf",38);
